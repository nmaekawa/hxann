...
WEBANN_TEMPLATE = {
    ...
    "body": {
        "type": "List",
        "items": [{"type": "TextualBody", "purpose": "commenting", "value": ""}],
    },
    "target": {
        "type": "List",
        "items": [
...
def translate_record(record, fmt):
    ...
    if START not in record or END not in record:
        raise HxannError("missing start or end times in row({})".format(record))
    if SOURCE not in record:
        raise HxannError("missing video link in row({})".format(record))

    # check if required keys have a value
    if (
        not record[START].strip()
        and not record[END].strip()
    ...
    if mimetype is None:
        raise HxannError("unknown video mimetype for link({})".format(record))
    ...
    if not record[END]:
        ...
        if end is None:
            raise HxannError(
                "malformed end time({}) in row({})".format(record[END], record["id"])
            )
        if end < record[START]:
            raise HxannError(
...
def webann_record(record):
    ...
    result["target"]["items"][0]["selector"]["items"][0]["value"] = "t={},{}".format(
